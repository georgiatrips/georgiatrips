...
export default function DatePicker({ value, onChange, placeholder = "თარიღი", direction = "down", availableDates = null }) {
  ...
  return (
    <div className="dp-wrap dp-wrap--form" ref={wrapRef}>
      <button
        type="button"
        className={`dp-trigger dp-trigger--form ${open ? "dp-trigger--open" : ""}`}
        onClick={() => setOpen(o => !o)}
        aria-label="კალენდრის გახსნა"
      >
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="dp-icon dp-icon--form">
        ...
        <svg className="dp-chevron dp-chevron--form" width="12" height="12" viewBox="0 0 24 24" fill="none"
      ...
      {open && (
        <div className={`dp-popover ${direction === "up" ? "dp-popover--up" : ""}`}>
          {/* Header */}
