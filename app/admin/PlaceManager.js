...
export default function PlaceManager() {
  ...
  const submit = async (event) => { event.preventDefault(); setSaving(true); setMessage(""); try { const payload = { ...form, title: form.title.trim(), desc: form.desc.trim(), img: form.gallery[0] || "/hero.png" }; if (editingId) await updatePlace(editingId, payload); else await createPlace(payload); setForm(empty); setEditingId(null); await refresh(); setMessage("ადგილი შენახულია"); } catch (error) { setMessage(error.message); } finally { setSaving(false); } };
