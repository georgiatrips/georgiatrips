...
export default function PostsPage() {
  ...
  const uploadPostImage = async (file) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || "ატვირთვა ვერ მოხერხდა");
      setPostImage(result.url);
    } catch (error) {
      setComposerMessage("ფოტოს ატვირთვა ვერ მოხერხდა.");
    } finally {
  ...
  const submitPost = async (event) => {
    ...
    if (!user) {
      setComposerMessage("პოსტის დასამატებლად გაიარეთ ავტორიზაცია.");
      return;
    }
    if (!composerText.trim() && !pollQuestion.trim()) {
      setComposerMessage("დაწერეთ ტექსტი ან შექმენით გამოკითხვა.");
      return;
    }
    setSavingPost(true);
    setComposerMessage("");
    try {
      ...
      setPostImage("");
      setComposerMessage("პოსტი გამოქვეყნდა.");
    } catch (error) {
      setComposerMessage("პოსტის გამოქვეყნება ვერ მოხერხდა. სცადეთ თავიდან.");
      console.error(error);
    } finally {
      setSavingPost(false);
    }
        ...
        <section className="posts-my-section">
          <div className="container">
            <div className="posts-my-articles posts-my-articles-top">
              <div className="posts-my-articles-heading"><h2>ჩემი სტატიები</h2><span>{remotePosts.filter((post) => post.authorId === user.uid).length}</span></div>
              {remotePosts.filter((post) => post.authorId === user.uid).length === 0 ? <p>ჯერ სტატია არ დაგიმატებიათ.</p> : (
                <div className="posts-my-articles-grid">
                  {remotePosts.filter((post) => post.authorId === user.uid).map((post) => (
                    <article key={post.id} className="posts-my-article">
                      ...
                      <div><button type="button" onClick={() => startEditPost(post)}>რედაქტირება</button><button type="button" onClick={() => setDeletePostTarget(post)}>წაშლა</button></div>
                    </article>
                  ))}
                </div>
              )}
          ...
          <aside className="posts-tours-sidebar">
            <div className="posts-sidebar-tour-list">
              {sidebarTours.map((tour) => (
                <Link key={tour.id} href={"/tours/" + tour.id} className="posts-sidebar-tour">
                  ...
                  <span className="posts-sidebar-tour-info"><strong>{asText(tour.title)}</strong><small>{asText(tour.duration, "ტური")} · {asText(tour.destinationLabel || tour.destination, "საქართველო")}</small></span>
                </Link>
              ))}
            </div>
              ...
              <div className="posts-composer-top">
                ...
                <span>{user ? (user.displayName || user.email?.split("@")[0] || "მომხმარებელი") : "პოსტის დასაწერად გაიარეთ ავტორიზაცია"}</span>
              </div>
              <textarea value={composerText} onChange={(e) => setComposerText(e.target.value)} placeholder="რას ფიქრობთ მოგზაურობაზე?" disabled={!user} />
              <div className="posts-photo-row">
                <label className="posts-photo-upload">
                  ...
                  {uploadingPostImage ? "ფოტო იტვირთება..." : "ფოტოს დამატება"}
                </label>
              ...
              {composerMode === "feeling" && (
                <div className="posts-composer-extra">
                  <label>განწყობა / აქტივობა</label>
                  <select value={feeling} onChange={(e) => setFeeling(e.target.value)}><option value="">აირჩიეთ განწყობა</option><option>😊 ბედნიერი</option><option>😍 აღფრთოვანებული</option><option>🤩 შთაგონებული</option><option>😎 თავგადასავლებისთვის მზად</option><option>🌿 მშვიდი</option></select>
                </div>
              )}
              {composerMode === "poll" && (
                <div className="posts-composer-extra">
                  <label>გამოკითხვა</label>
                  <input value={pollQuestion} onChange={(e) => setPollQuestion(e.target.value)} placeholder="დაწერეთ კითხვა..." />
                  {pollOptions.map((option, index) => <input key={index} value={option} onChange={(e) => setPollOptions((prev) => prev.map((item, i) => i === index ? e.target.value : item))} placeholder={"პასუხი " + (index + 1)} />)}<button type="button" className="posts-add-poll-option" onClick={() => setPollOptions((prev) => [...prev, ""])}>+ პასუხის დამატება</button>
                </div>
              )}
              <div className="posts-composer-actions">
                <button type="button" onClick={() => setComposerMode(composerMode === "feeling" ? null : "feeling")}>განწყობა / აქტივობა</button>
                <button type="button" onClick={() => setComposerMode(composerMode === "poll" ? null : "poll")}>გამოკითხვა</button>
                {user && <button type="submit" disabled={savingPost}>{savingPost ? "იგზავნება..." : editingPostId ? "ცვლილების შენახვა" : "გამოქვეყნება"}</button>}
              </div>
            ...
            {loading ? (
              <div className="posts-loading-state">
                ...
                <p>სტატიები იტვირთება...</p>
              </div>
            ) : postsData.length === 0 ? <div className="posts-empty-state"><h2>სტატიები მალე დაემატება</h2><p>თვალი ადევნეთ ჩვენს ახალ მოგზაურობის ისტორიებს.</p></div> : postsData.map((post) => {
              ...
              <article key={post.id} className="facebook-post-card">
                  ...
                  {post.poll?.question && (
                    <div className="posts-poll-card">
                          ...
                          >
                            ...
                            {hasVoted && (
                              <span className="posts-poll-option-result">
                                ...
                                <span className="posts-poll-option-count">{count} ხმა</span>
                              </span>
                            )}
                          </button>
                        );
                      ...
                      <div className="posts-poll-footer">
                        <span>{post.poll.totalVotes || 0} ხმა</span>
                        {!user && <span>ხმის მისაცემად გაიარეთ ავტორიზაცია</span>}
                        {user && post.poll.userVoteIndex === null && <span>აირჩიეთ პასუხი</span>}
                        {user && post.poll.userVoteIndex !== null && post.poll.userVoteIndex !== undefined && <span>თქვენი არჩევანი აღნიშნულია ✓</span>}
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="fb-reactions-bar">
                  ...
                  <div className="fb-counts-group">
                    <span className="fb-count-item">{userComments.length} კომენტარი</span>
                    <span className="fb-dot">•</span>
      ...
      {deletePostTarget && (
        <div className="posts-delete-overlay" role="dialog" aria-modal="true">
          <div className="posts-delete-modal">
            ...
            <h2>სტატიის წაშლა?</h2>
            <p>ეს მოქმედება ვეღარ დაბრუნდება. ნამდვილად გსურთ „{deletePostTarget.title}“-ის წაშლა?</p>
            <div className="posts-delete-actions">
              <button type="button" className="posts-delete-cancel" onClick={() => setDeletePostTarget(null)}>გაუქმება</button>
              <button type="button" className="posts-delete-confirm" onClick={removePost}>წაშლა</button>
            </div>
          </div>
        </div>
      )}
